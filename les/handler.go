...
import (
	...
	"github.com/clanetcnc/go-clanetdiuma/core/state"
	...
	"github.com/clanetcnc/go-clanetdiuma/trie"
)
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light Clanetdium protocol")
	...
	log.Info("Light Clanetdium protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Light Clanetdium peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, genesis.Hash(), pm.server); err != nil {
		p.Log().Debug("Light Clanetdium handshake failed", "err", err)
	...
	if err := pm.peers.Register(p); err != nil {
		clientErrorMeter.Mark(1)
		p.Log().Error("Light Clanetdium peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light Clanetdium message handling failed", "err", err)
			if p.fcServer != nil {
				p.fcServer.DumpLogs()
			}
			return err
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light Clanetdium message arrived", "code", msg.Code, "bytes", msg.Size)

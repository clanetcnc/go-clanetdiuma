...
{{end}}
RUN \
  echo 'gcnc --cache 512 init /genesis.json' > gcnc.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.clanetdium/keystore/ && cp /signer.json /root/.clanetdium/keystore/' >> gcnc.sh && \{{end}}
	echo $'exec gcnc --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--miner.etherbase {{.Etherbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> gcnc.sh

ENTRYPOINT ["/bin/sh", "gcnc.sh"]
`
...
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.clanetdium{{if .Ethashdir}}
      - {{.Ethashdir}}:/root/.cnchash{{end}}
    environment:
      - PORT={{.Port}}/tcp
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.clanetdium"],
		ethashdir:  infos.volumes["/root/.cnchash"],
		port:       port,
